...
def add_varying_noise(data, noise_factor_range=(0.01, 0.5)):
    """
...
def augment_data(data, labels, weights, event_indices=None, apply_mixup=False, mixup_data=None, params=None, sampling_rate=1250):
    ...
    # Add low-frequency oscillations (theta, beta) as negative examples
    # data = add_lower_frequency_noise(data)

    # Apply other augmentations as needed
    # print(data.shape)
    ...
    # data = add_long_sinusoidal_noise(data)
    # print(data.shape)
    data = random_channel_shuffle(data)
    # print(data.shape)
    data = random_channel_dropout(data)
    # print(data.shape)
...
def rippleAI_load_dataset(params, mode='train', preprocess=True, spatial_freq=1250):
    ...
    # augmented_dataset = apply_augmentation_to_dataset(train_dataset, params=params)
    # pdb.set_trace()
    # aa = next(iter(augmented_dataset))
