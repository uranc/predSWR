...
def rippleAI_load_dataset(params, mode='train', preprocess=True, use_band=None):
    ...
    def to_channels_first(x, y, w=None):
        # x starts as (Batch, Time, Channels) -> Flip to (Batch, Channels, Time)
        x_transposed = tf.transpose(x, perm=[0, 2, 1])
        if w is not None:
            return x_transposed, y, w
        return x_transposed, y

    # Apply the flip just before returning
    train_dataset = train_dataset.map(to_channels_first, num_parallel_calls=tf.data.AUTOTUNE)
    test_dataset = test_dataset.map(to_channels_first, num_parallel_calls=tf.data.AUTOTUNE)   
