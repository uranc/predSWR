...
for prob in prob_list:
    ...
    plt.ylabel('Rate')
    plt.title('TPR and FPR vs. Threshold {0}'.format(prob))
    plt.legend(loc="best")
    ...
    recall.append(recall_aux)
    precision.append(precision_aux)
    f1.append(f1_aux)
# pdb.set_trace()
# # Ploting metrics
...
for i in range(len(prob_list)):
    plt.plot(fpr[i], tpr[i], lw=2, label='{0}'.format(prob_list[i]))
...
# Saving images 
directory =  '/cs/projects/OWVinckSWR/DL/predSWR/time_analysis/ROC_curves/'   
if not os.path.exists(directory):
    os.makedirs(directory)
plt.savefig(directory + '{0}.png'.format(prob))
