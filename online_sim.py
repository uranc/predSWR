...
#### RIPPL_AI_1D ####
    ...
    # model.compile(loss="binary_crossentropy", optimizer=optimizer)
    from keras.utils import timeseries_dataset_from_array   
    sample_length = timesteps #params['NO_TIMEPOINTS']*2 
    ...
    for LFP in val_datasets:
        ...
        #     probs = np.hstack((np.zeros((sample_length-1, 1)), windowed_signal))
    ...
    if save_prob == 'True' :
        # Saving the probabilities
        directory =  '/cs/projects/OWVinckSWR/DL/predSWR/online/' 
        if not os.path.exists(directory):
            os.makedirs(directory)
        # Saving the probabilities in a .npy
        for i, array in enumerate(val_pred_rippAI): np.save(directory + 'online_prob_rippAI_{0}_{1}.npy'.format(arch, i), np.array(array))
    
    directory =  '/cs/projects/OWVinckSWR/DL/predSWR/online/online_images/imagesrippAI/{0}/'.format(arch)   
    if not os.path.exists(directory):
        os.makedirs(directory)
